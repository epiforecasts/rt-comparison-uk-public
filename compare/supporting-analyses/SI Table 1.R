...
median_after_1 <- samples %>%
  ...
  summarise(median = median(value.x),
            lower_90 = quantile(value.x, 0.9),
            upper_90 = quantile(value.x, 0.1)) %>%
  as_tibble()
...
min_after_1 <- summary_after_1 %>%
  ...
  mutate(value = paste0(as.character(date), 
                      " (", round(median, 2), 
                      " [",round(lower_90, 2), 
                      "-", round(upper_90, 2), "])"),
         name = "min_after_1") %>%
...
names(table1) <- c("Region",
                   "Source",
                   "Date median first crossed 1",
                   "Median and 90% CrI, date after median crossed 1 to most recent estimate",
                   "Number of times a single estimate crosses the series median",
                   "Minimum median estimate from when median crossed 1 to last estimate (date, median [90%CrI])",
                   "Maximum median estimate, from when median crossed 1 to last estimate (date, median [90%CrI])",
                   "Duration of days between successive troughs (mean, 95% CI)")
readr::write_csv(table1, "compare/rt-comparison/si-table-1.csv")
